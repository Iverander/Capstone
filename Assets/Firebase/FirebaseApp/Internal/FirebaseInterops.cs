    ...
    internal static class FirebaseInterops
    {
        ...
        private static readonly Lazy<string> _sdkVersionFetcher = new(() =>
        {
            try
            {
                ...
                var versionInfoType = typeof(FirebaseApp).Assembly.GetType("Firebase.VersionInfo");
                ...
                var sdkVersionProperty = versionInfoType.GetProperty(
                    "SdkVersion",
      ...
      UnityEngine.Debug.LogError(message);
        ...
        {
            if (firebaseApp == null || _dataCollectionProperty == null) return false;
        ...
        private static void InitializeAppCheckReflection()
        {
            ...
            try
            {
                ...
                var appCheckTokenTaskType = _appCheckGetTokenMethod.ReturnType;
                ...
                var appCheckTokenType = _appCheckTokenResultProperty.PropertyType;
        ...
        internal static async Task<string> GetAppCheckTokenAsync(FirebaseApp firebaseApp)
        {
            ...
            if (!_appCheckReflectionInitialized) return null;

            try
            {
                // Get the FirebaseAppCheck instance for the current FirebaseApp
                var appCheckInstance = _appCheckGetInstanceMethod.Invoke(null, new object[] { firebaseApp });
                ...
                var taskObject = _appCheckGetTokenMethod.Invoke(appCheckInstance, new object[] { false });
                ...
                // Get the Result property from the Task<AppCheckToken>
                var tokenResult =
                    _appCheckTokenResultProperty.GetValue(appCheckTokenTask); // This is the AppCheckToken struct
                if (tokenResult == null)
                {
        ...
        private static void InitializeAuthReflection()
        {
            ...
            try
            {
                ...
                _authCurrentUserProperty =
                    _authType.GetProperty("CurrentUser", BindingFlags.Instance | BindingFlags.Public);
                ...
                var userType = _authCurrentUserProperty.PropertyType;
                ...
                var tokenTaskType = _userTokenAsyncMethod.ReturnType;
        ...
        internal static async Task<string> GetAuthTokenAsync(FirebaseApp firebaseApp)
        {
            ...
            if (!_authReflectionInitialized) return null;

            try
            {
                // Get the FirebaseAuth instance for the given FirebaseApp.
                var authInstance = _authGetAuthMethod.Invoke(null, new object[] { firebaseApp });
                ...
                var currentUser = _authCurrentUserProperty.GetValue(authInstance);
                if (currentUser == null)
                    // No user logged in, so no token
                    return null;

                // Invoke TokenAsync(false) - returns a Task<string>
                var taskObject = _userTokenAsyncMethod.Invoke(currentUser, new object[] { false });
        ...
        internal static async Task AddFirebaseTokensAsync(HttpRequestMessage request, FirebaseApp firebaseApp,
            string authTokenPrefix = "Firebase")
        {
            var appCheckToken = await GetAppCheckTokenAsync(firebaseApp);
            if (!string.IsNullOrEmpty(appCheckToken)) request.Headers.Add(appCheckHeader, appCheckToken);

            var authToken = await GetAuthTokenAsync(firebaseApp);
            if (!string.IsNullOrEmpty(authToken)) request.Headers.Add(authHeader, $"{authTokenPrefix} {authToken}");
        }
        ...
        {
            var appCheckToken = await GetAppCheckTokenAsync(firebaseApp);
            if (!string.IsNullOrEmpty(appCheckToken)) socket.Options.SetRequestHeader(appCheckHeader, appCheckToken);
