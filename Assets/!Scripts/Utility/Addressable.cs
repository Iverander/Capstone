    ...
    namespace Utility
    {
        ...
        public static class Addressable
        {
            ...
            public static async Task<List<T>> LoadAssets<T>(string label) where T : Object
            {
                ...
                if (handle.Status == AsyncOperationStatus.Succeeded) return (List<T>)handle.Result;
            ...
            {
                var item = await LoadAsset<GameObject>(addressable);

                if (item == null) Debug.LogError("[Addressable] item is null");
                if (getType) return item.GetComponent<T>();
            ...
            public static async Task<T> LoadAsset<T>(string addressable) where T : Object
            {
                ...
                if (handle.Status == AsyncOperationStatus.Succeeded) return (T)handle.Result;
