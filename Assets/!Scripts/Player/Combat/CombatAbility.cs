...
namespace Capstone
{
    ...
    public class CombatAbility : Ability
    {
        ...
        [Header("Targeting")] [SerializeField] protected Vector3 size = Vector3.one;

        [field: SerializeField] protected Vector3 center { get; private set; } = Vector3.up + Vector3.forward;

        [Space] [SerializeField] private LayerMask layerToHit;

        [SerializeField] protected bool includeSelf;

        [Header("Stats")] [SerializeField] protected float damage = 10;
        ...
        protected override void Action()
        {
            ...
            {
                if (damage > 0) creature.health.Damage(damage * origin.stats.DamageMultiplier);

                if (knockbackForce > 0)
                {
                    creature.StartCoroutine(creature.Stun(duration));
                    creature.rb.AddForce(
                        (creature.transform.position - origin.transform.position) * (knockbackForce * 10),
                        ForceMode.Force);
                }
            }
        }
