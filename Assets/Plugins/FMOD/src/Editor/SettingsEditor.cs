...
namespace FMODUnity
{
    ...
    public class SettingsEditor : Editor
    {
        ...
        private static readonly string[] ToggleDisplay =
            { L10n.Tr("Disabled"), L10n.Tr("Enabled"), L10n.Tr("Development Build Only") };

        private static readonly string[] OverlayLocations =
        {
            L10n.Tr("TopLeft"), L10n.Tr("TopCenter"), L10n.Tr("TopRight"), L10n.Tr("BottomLeft"),
            L10n.Tr("BottomCenter"), L10n.Tr("BottomRight"), L10n.Tr("Center"), L10n.Tr("VR")
        };

        private static readonly string[] FrequencyDisplay =
        {
        ...
        private static readonly int[] FrequencyValues = { 0, 22050, 24000, 32000, 44100, 48000 };
        ...
        private static readonly string[] ThreadTypeDisplay =
        {
            L10n.Tr("Mixer"),
            L10n.Tr("Feeder"),
            L10n.Tr("Stream"),
        ...
        private static readonly int[] BankLoadTypeValues =
        {
        ...
        private static readonly string[] BankLoadTypeDisplay =
        {
            L10n.Tr("All"),
            L10n.Tr("Specified"),
            L10n.Tr("None")
        ...
        private static readonly int[] MeterChannelOrderingeValues =
        {
        ...
        private static readonly string[] MeterChannelOrderingeDisplay =
        {
            L10n.Tr("Standard"),
            L10n.Tr("SeparateLFE"),
            L10n.Tr("Positional")
        ...
        private static readonly GUIContent BankRefreshLabel = new(L10n.Tr("Refresh Banks"));

        private static readonly GUIContent[] BankRefreshCooldownLabels =
        {
            new(L10n.Tr("After 1 second")),
            new(L10n.Tr("After 5 seconds")),
            new(L10n.Tr("After 10 seconds")),
            new(L10n.Tr("After 20 seconds")),
            new(L10n.Tr("After 30 seconds")),
            new(L10n.Tr("After 1 minute")),
            new(L10n.Tr("Prompt Me")),
            new(L10n.Tr("Manually"))
        };
        ...
        private void OnEnable()
        {
            ...
            {
                if (dynamicPluginsView.platform is PlatformPlayInEditor) EditorUtils.RecreateSystem();
            };
            ...
            Platform.PropertyOverrideControl[] properties)
        {
            if (data.hasParent)
            {
                    ...
                    && data.labelRect.Contains(Event.current.mousePosition))
                {
                    ...
                    var revertContent = new GUIContent(L10n.Tr("Revert"));
            ...
            Platform.PropertyAccessor<ScreenPosition> property)
        {
            ...
            if (EditorGUI.EndChangeCheck()) property.Set(platform, (ScreenPosition)next);
        }
        ...
        private void DisplayDebugFontSelection(string label, Platform platform, Platform.PropertyAccessor<int> property)
        {
            ...
            if (EditorGUI.EndChangeCheck()) property.Set(platform, Mathf.Min(next, 20));
        }
        ...
        private void DisplayThreadAffinityGroups(Platform platform)
        {
            ...
            var anyButtonContent = new GUIContent(L10n.Tr("Any"));
            ...
            GUIStyle affinityStyle, float affinityWidth, out bool delete)
        {
            ...
            var editButtonContent = new GUIContent(L10n.Tr("Edit"));
            ...
            if (editable)
            {
                ...
                var deleteButtonContent = new GUIContent(L10n.Tr("Delete"));
        ...
        {
            if (platform is PlatformGroup || platform is PlatformDefault) return;
                ...
                using (new EditorGUI.IndentLevelScope())
                {
                    ...
                    using (new EditorGUI.DisabledScope(!editable))
                    {
                        foreach (var channelCount in platform.CodecChannels)
                        {
                            ...
                            var channels = EditorGUILayout.IntSlider(channelCount.format.ToString(),
        ...
        {
            var rect = EditorUtils.DrawHelpButtonLayout(() =>
                new SimpleHelp(L10n.Tr(
                    "Select the speaker mode that matches the project platform settings in the FMOD Studio build preferences.")));
        ...
        private void DisplayLiveUpdatePort(string label, Platform platform, Platform.PropertyAccessor<int> property)
        {
            ...
            var resetContent = new GUIContent(L10n.Tr("Reset"));
        ...
        private void DisplayPlatform(Platform platform)
        {
            ...
            using (new EditorGUI.IndentLevelScope())
            {
                ...
                if (platform.IsLiveUpdateEnabled)
                    DisplayLiveUpdatePort(L10n.Tr("Live Update Port"), platform,
                        Platform.PropertyAccessors.LiveUpdatePort);

                DisplayTriStateBool(L10n.Tr("Debug Overlay"), platform, Platform.PropertyAccessors.Overlay);
                if (platform.IsOverlayEnabled)
                {
                    DisplayPositionRect(L10n.Tr("Debug Location"), platform,
                        Platform.PropertyAccessors.OverlayPosition);
                    DisplayDebugFontSelection(L10n.Tr("Font size"), platform,
                        Platform.PropertyAccessors.OverlayFontSize);
                }
                ...
                else if (platform is PlatformDefault) DisplaySpeakerMode(L10n.Tr("Speaker Mode"), platform);

                DisplayCallbackHandler(L10n.Tr("Callback Handler"), platform);

                DisplayInt(L10n.Tr("Virtual Channel Count"), platform, Platform.PropertyAccessors.VirtualChannelCount,
                    1, 2048);
                DisplayInt(L10n.Tr("Real Channel Count"), platform, Platform.PropertyAccessors.RealChannelCount, 1,
                    256);
                ...
                DisplayPlugins(L10n.Tr("Static Plugins"), staticPluginsView, platform, ref expandStaticPlugins,
                    warning);
        ...
        private void DisplayPlatformHeader(Platform platform)
        {
            ...
            if (platform.Parent != null || platform is PlatformPlayInEditor)
            {
                ...
                {
                    labelContent = new GUIContent(string.Format(
                        "<b>{0}</b>: {1} " + L10n.Tr("inheriting from Unity build target: "),
                ...
                {
                    labelContent = new GUIContent(string.Format("<b>{0}</b>: {1} " + L10n.Tr("inheriting from"),
                        platform.DisplayName, type));
        ...
        {
            var rect = DrawPlatformPropertyLabel(L10n.Tr("DSP Buffer Settings"), platform,
        ...
        private bool DisplayAutoDSPBufferSettings(Rect rect, Platform platform)
        {
            ...
            var content = new GUIContent(L10n.Tr("Auto"));
        ...
        private void DisplayDSPBufferFields(Platform platform)
        {
            ...
            using (new EditorGUI.IndentLevelScope())
            {
                ...
                var nextLength =
                    Mathf.Max(EditorGUILayout.IntField(L10n.Tr("DSP Buffer Length"), lengthProperty.Get(platform)), 8);
                var nextCount =
                    Mathf.Max(EditorGUILayout.IntField(L10n.Tr("DSP Buffer Count"), countProperty.Get(platform)), 2);
        ...
        protected override void OnHeaderGUI()
        {
            ...
            var text = new GUIContent(L10n.Tr("FMOD Settings"));

            var textSize = mainHeaderStyle.CalcSize(text);

            var iconSize = GUI.skin.label.CalcSize(mainHeaderIcon);

            var rect = EditorGUILayout.GetControlRect(false, Math.Max(textSize.y, iconSize.y) * 1.75f);
        ...
        private void DrawSourceSelection(string invalidSourceMessage)
        {
            ...
            if (sourceType == SourceType.FMODStudioProject)
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    EditorGUI.BeginChangeCheck();
                    var newPath =
                        EditorGUILayout.TextField(L10n.Tr("Studio Project Path"), sourceProjectPath.stringValue);
                    if (EditorGUI.EndChangeCheck())
        ...
        {
            if (BrowseForSourceBankPath(serializedObject, hasPlatforms.boolValue)) Repaint();
        }
        ...
        private void DrawTargetSelection()
        {
            ...
            var newImportTypeIndex = EditorGUILayout.Popup(L10n.Tr("Import Type"), importTypeIndex, importTypeNames);
            ...
            if (newImportTypeIndex != importType.enumValueIndex)
            {
                var deleteBanks = EditorUtility.DisplayDialog(
                    ...
                    string.Format(L10n.Tr("Do you want to delete the {0} banks in {1} "),
                        importTypeNames[importTypeIndex], settings.TargetPath),
        ...
        private void DrawInitializationSection()
        {
            if (DrawSectionHeaderLayout(Section.Initialization, L10n.Tr("Initialization")))
                using (new EditorGUI.IndentLevelScope())
                {
                    ...
                    EditorGUILayout.PropertyField(enableMemoryTracking,
                        new GUIContent(L10n.Tr("Enable Memory Tracking")));
                    ...
                    using (new EditorGUI.DisabledScope(importType.intValue == (int)ImportType.AssetBundle))
                    {
                        ...
                        using (new EditorGUI.DisabledScope(bankLoadType.intValue == (int)BankLoadType.None))
                        {
                            EditorGUILayout.PropertyField(automaticSampleLoading,
                                new GUIContent(L10n.Tr("Load Bank Sample Data")));
                        }
        ...
        private void BrowseForBankToLoad()
        {
            ...
            var path = EditorUtility.OpenFilePanel(L10n.Tr("Locate Bank"), bankDirectory, "bank");
            ...
            bool inInspector)
        {
            ...
            using (new NoIndentScope())
            {
                ...
                if (cooldown.intValue >= 0)
                {
                    ...
                    showWindow.boolValue =
                        EditorGUI.ToggleLeft(toggleRect, L10n.Tr("Show Status Window"), showWindow.boolValue);
                }
            }
        }
        ...
        public static bool BrowseForSourceProjectPath(SerializedObject serializedObject)
        {
            ...
            var newPath = EditorUtility.OpenFilePanel(L10n.Tr("Locate Studio Project"), sourceProjectPath.stringValue,
                "fspro");
        ...
        public static bool BrowseForSourceBankPath(SerializedObject serializedObject, bool multiPlatform = false)
        {
            ...
            var newPath =
                EditorUtility.OpenFolderPanel(L10n.Tr("Locate Build Folder"), sourceBankPath.stringValue, null);
        ...
        private void DrawLinkageSettings()
        {
            ...
            if (EditorGUI.EndChangeCheck()) HandleGUIDOnlySerializationToggle(serializeGUIDsOnly.boolValue);
        }

        private void HandleGUIDOnlySerializationToggle(bool oldValue)
        {
            var action = oldValue ? "Enable" : "Disable";
            var content =
        ...
        private class ThreadListEditor : EditorWindow
        {
            ...
            private void OnGUI()
            {
                ...
                foreach (ThreadType thread in Enum.GetValues(typeof(ThreadType)))
                {
                    ...
                    var include = EditorGUI.ToggleLeft(threadRect, ThreadTypeDisplay[(int)thread],
                        group.threads.Contains(thread));
            ...
            private static Vector2 CalculateSize()
            {
                ...
                {
                    var size = ThreadStyle.CalcSize(new GUIContent(ThreadTypeDisplay[(int)thread]));
        ...
        private class ProjectPlatformSelector : PopupWindowContent
        {
            ...
            private readonly GUIContent speakerModeHeader = new(L10n.Tr("Surround speaker mode:"));
            private readonly Vector2 speakerModeSize;

            private readonly GUIContent subdirectoryHeader = new(L10n.Tr("Output sub-directory:"));
            ...
            public override void OnGUI(Rect rect)
            {
                ...
                EditorUtils.DrawHelpButton(helpButtonRect,
                    () => new SimpleHelp(L10n.Tr(
                        "Select the output sub-directory and surround speaker mode that match the project platform settings in the FMOD Studio build preferences.")));
            }
        }

        private class SourceSelectionHelp : HelpContent
        {
            private readonly GUIContent introduction = new(L10n.Tr("Choose how to access your FMOD Studio content:"));

            private readonly ListEntry[] listEntries =
            {
                new(L10n.Tr("FMOD Studio Project"),
                    L10n.Tr("If you have the complete FMOD Studio project.")
                ),
                new(L10n.Tr("Single Platform Build"),
                    L10n.Tr("If you have the contents of the <b>Build</b> folder for a single platform.")
                ),
                new(L10n.Tr("Multiple Platform Build"),
                    L10n.Tr(
                        "If you have the contents of the <b>Build</b> folder for multiple platforms, with each platform in its own subdirectory.")
                )
            };
        ...
        private class PlatformsView : TreeView
        {
            ...
            private void AddPlatform(object data)
            {
                ...
                Undo.RecordObjects(new Object[] { settings, platform, platform.Parent }, L10n.Tr(UndoMessage));
            ...
            private void AddGroup()
            {
                ...
                Undo.RecordObjects(new Object[] { settings, settings.DefaultPlatform }, L10n.Tr(UndoMessage));

                var sortOrder = UpdateSortOrderForChildren(settings.DefaultPlatform, null, L10n.Tr(UndoMessage));

                var group = EditorSettings.Instance.AddPlatformGroup(L10n.Tr("New Group"), sortOrder);
            ...
            private void DeleteSelectedPlatform()
            {
                ...
                if (platform == null || platform == settings.DefaultPlatform ||
                    platform == settings.PlayInEditorPlatform) return;


                const string UndoMessage = "Delete FMOD Platform";

                Undo.RecordObjects(new Object[] { platform, platform.Parent, settings }, L10n.Tr(UndoMessage));
                ...
                while (platform.ChildIdentifiers.Count > 0)
                {
                    ...
                    SetPlatformParent(L10n.Tr(UndoMessage), settings, child, platform.Parent,
        ...
        private class PlatformPropertyStringListView : UnityEditorInternal.ReorderableList
        {
            ...
            private void DrawElement(Rect rect, int index, bool active, bool focused)
            {
                using (new NoIndentScope())
                {
                    ...
                    var newValue = EditorGUI.DelayedTextField(rect, list[index] as string);
