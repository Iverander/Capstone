...
namespace FMOD.Studio
{
    ...
    public struct System
    {
        ...
        public RESULT getParameterLabelByName(string name, int labelindex, out string label)
        {
            ...
            using (var encoder = StringHelper.GetFreeHelper())
            {
                ...
                if (result == RESULT.ERR_TRUNCATED)
                {
                    ...
                    result = FMOD_Studio_System_GetParameterLabelByName(handle, nameBytes, labelindex, stringMem,
        ...
        {
            return FMOD_Studio_System_LoadBankMemory(handle, buffer, length, LOAD_MEMORY_MODE.LOAD_MEMORY, flags,
                out bank.handle);
        }
    ...
    public struct EventDescription
    {
        ...
        public RESULT getParameterLabelByIndex(int index, int labelindex, out string label)
        {
            ...
            using (var encoder = StringHelper.GetFreeHelper())
            {
                var stringMem = Marshal.AllocHGlobal(256);
                var retrieved = 0;
                var result = FMOD_Studio_EventDescription_GetParameterLabelByIndex(handle, index, labelindex, stringMem,
                    256, out retrieved);

                if (result == RESULT.ERR_TRUNCATED)
                {
                    Marshal.FreeHGlobal(stringMem);
                    result = FMOD_Studio_EventDescription_GetParameterLabelByIndex(handle, index, labelindex,
        ...
        public RESULT getParameterLabelByName(string name, int labelindex, out string label)
        {
            ...
            using (var encoder = StringHelper.GetFreeHelper())
            {
                ...
                var retrieved = 0;
                var nameBytes = encoder.byteFromStringUTF8(name);
                var result = FMOD_Studio_EventDescription_GetParameterLabelByName(handle, nameBytes, labelindex,
    ...
    public struct EventInstance
    {
        ...
        public RESULT setParameterByIDWithLabel(PARAMETER_ID id, string label, bool ignoreseekspeed = false)
        {
            using (var encoder = StringHelper.GetFreeHelper())
            {
                return FMOD_Studio_EventInstance_SetParameterByIDWithLabel(handle, id,
